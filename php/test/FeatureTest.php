...
class FtCtrl extends MarketDataControl
{
...
class FtClient
{
    ...
    private MarketDataUtility $utility;

    public function __construct(MarketDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): MarketDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MarketDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MarketDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MarketDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MarketDataConfig::make_feature($name);
    ...
    private static function buildUrl(MarketDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MarketDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MarketDataResponse($fetched);
        }
    ...
    private function fail(MarketDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MarketDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
class MarketDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = MarketDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MarketDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!MarketDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, MarketDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return MarketDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = MarketDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = MarketDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = MarketDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MarketDataError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = MarketDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MarketDataError("graphql_error",
                "MarketDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }

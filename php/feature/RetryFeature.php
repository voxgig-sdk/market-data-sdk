...
class MarketDataRetryFeature extends MarketDataBaseFeature
{
    ...
    public function init(MarketDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MarketDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MarketDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MarketDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

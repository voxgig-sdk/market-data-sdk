...
class MarketDataEntity
{
    ...
    {
        if ($args) {
            $this->_data = MarketDataHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = MarketDataHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param MarketDataLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed MarketDataLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return MarketData|array The loaded MarketData as an assoc-array at the
     *   SDK boundary; throws MarketDataError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MarketDataHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

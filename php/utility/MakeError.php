...
class MarketDataMakeError
{
    public static function call(?MarketDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MarketDataContext([], null);
        }
        $op = $ctx->op ?? new MarketDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MarketDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MarketDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MarketDataError('', $msg, $ctx);

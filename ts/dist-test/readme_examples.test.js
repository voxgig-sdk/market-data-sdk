...
const TEST_SEED = { "entity": { "market_data": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+MarketDataSDK\b/.test(code) ||
        /\bMarketDataSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+MarketDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/MarketDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);

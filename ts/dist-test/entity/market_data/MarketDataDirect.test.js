...
(0, node_test_1.describe)('MarketDataDirect', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when MARKET_DATA_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('MARKET_DATA_TEST_LIVE'));
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.MarketDataSDK({
    ...
    (0, node_test_1.test)('direct-load-market_data', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-market_data', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.MARKET_DATA_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.MarketDataSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['MARKET_DATA_TEST_MARKET_DATA_ENTID'];
    ...
    const client = new __1.MarketDataSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=MarketDataDirect.test.js.map
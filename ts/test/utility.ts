...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MARKETDATA_TEST_LIVE ||
    'TRUE' === process.env.MARKETDATA_TEST_OVERRIDE
  ) {
  ...
  m.MARKETDATA_TEST_EXPLAIN = process.env.MARKETDATA_TEST_EXPLAIN || m.MARKETDATA_TEST_EXPLAIN

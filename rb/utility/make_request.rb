...
module MarketDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MarketDataResponse.new({})
    ...
    if fetch_err
      ...
      response = MarketDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MarketDataResponse.new(fetched)
    else

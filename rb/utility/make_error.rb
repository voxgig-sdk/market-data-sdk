...
module MarketDataUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = MarketDataContext.new({}, nil)
    end
    op = ctx.op || MarketDataOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || MarketDataResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(MarketDataError) ? err.msg : err.to_s
    ...
    sdk_err = MarketDataError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(MarketDataError)

...
class MarketDataEntity
  ...
  def make
    opts = @_entopts.dup
    MarketDataEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MarketDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MarketDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.MarketData.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [MarketData, Hash] the loaded MarketData; raises MarketDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MarketDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

...
require_relative 'MarketData_types'
...
class MarketDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MarketDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MarketDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MarketDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MarketDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MarketDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MarketDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MarketDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MarketDataError.new(
        "#{op}_allow",
        "MarketDataSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MarketDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MarketDataError.new(
        "graphql_error", "MarketDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

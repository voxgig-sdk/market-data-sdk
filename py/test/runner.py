...
class MarketDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MarketDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MarketDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MarketDataTestRunner.getenv("MARKET_DATA_TEST_LIVE")
        override = MarketDataTestRunner.getenv("MARKET_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MarketDataTestRunner.getenv(key)
        ...
        explain = MarketDataTestRunner.getenv("MARKET_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MarketDataTestRunner._test_control is not None:
            return MarketDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MarketDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MarketDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MarketDataTestRunner.load_test_control()
...
def load_env_local():
    MarketDataTestRunner.load_env_local()


def env_override(m):
    return MarketDataTestRunner.env_override(m)


def entity_data(v):
    return MarketDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MarketDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MarketDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MarketDataTestRunner.load_test_control()


def live_client_options():
    return MarketDataTestRunner.live_client_options()


def live_delay_ms():
    return MarketDataTestRunner.live_delay_ms()

...
{
    private static function utility(): MarketDataUtility
    {
        return new MarketDataUtility();
    }
    ...
    private static function ctx(array $over = []): MarketDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MarketDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MarketDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MarketDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MarketDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MarketDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MarketDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MarketDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MarketDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MarketDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MarketDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MarketDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new MarketDataResult([]),
        ]);
        [, $err] = MarketDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new MarketDataSpec([]),
            'response' => null,
            'result' => new MarketDataResult([]),
        ]);
        [, $err] = MarketDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MarketDataSpec(['step' => 's']),
            'response' => new MarketDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MarketDataResult([]),
        ]);
        [, $err] = MarketDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MarketDataSpec(['step' => 's']),
            'response' => new MarketDataResponse(self::resp(200, ['v' => 1])),
            'result' => new MarketDataResult([]),
        ]);
        [, $err] = MarketDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MarketDataSpec(['step' => 's']),
            'response' => new MarketDataResponse(self::resp(200, ['v' => 2])),
            'result' => new MarketDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MarketDataMakeResponse::call($ctx);
    ...
    {
        $preset = new MarketDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MarketDataSpec([]),
            'response' => new MarketDataResponse([]),
            'result' => new MarketDataResult([]),
        ...
        [$got, $err] = MarketDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MarketDataResult([])]);
        [, $err] = MarketDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MarketDataSpec([]), 'result' => null]);
        [, $err] = MarketDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MarketDataSpec(['step' => 's']),
            'result' => new MarketDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MarketDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MarketDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MarketDataSpec(['step' => 's']),
            'result' => new MarketDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MarketDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MarketDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MarketDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MarketDataSpec([]),
            'result' => new MarketDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MarketDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MarketDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MarketDataError('boom', 'boom');
        $utility->fetcher = function (MarketDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MarketDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MarketDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MarketDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MarketDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MarketDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        MarketDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MarketDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MarketDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MarketDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MarketDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MarketDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MarketDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MarketDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MarketDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MarketDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MarketDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MarketDataResult(['ok' => false])]);
        $this->expectException(MarketDataError::class);
        MarketDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MarketDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MarketDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        MarketDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MarketDataBaseFeature();
        $b = new MarketDataBaseFeature();
        MarketDataFeatureAdd::call($ctx, $a);
        MarketDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MarketDataBaseFeature
    {
        $f = new MarketDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MarketDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MarketDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MarketDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MarketDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        MarketDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MarketDataResponse(['headers' => null]),
            'result' => new MarketDataResult([]),
        ]);
        MarketDataResultHeaders::call($ctx);

...
boru-driven command-line client **and** interactive REPL for the MarketData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/market-data-cli)
make build

# 2. See usage (words, entities, env vars)
./market-data-cli --help

# 3. Provide credentials once, via the environment
export MARKET_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./market-data-cli load 1 market_data            # {id:1} shorthand
./market-data-cli load '{id:1}' market_data       # explicit match map

# 5. Override the API base URL for a single call
MARKET_DATA_BASE=https://api.example.com ./market-data-cli load 1 market_data

# 6. No arguments -> interactive REPL
./market-data-cli
market-data> load 1 market_data
market-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/market-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/market-data-cli load 1 market_data
   ```
...
```sh
./market-data-cli load 1 market_data          # scalar shorthand for {id:1}
./market-data-cli load '{id:1}' market_data     # explicit match map
```
...
./market-data-cli load 1 market_data
```
...
Run with no arguments to open a REPL (prompt `market-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `market_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/market-data-cli`. |
...
(`./market-data-cli <expr>`) and interactively (the REPL), and expressions compose the

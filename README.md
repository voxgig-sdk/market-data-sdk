...
Market Data API client, generated from the OpenAPI spec.

This API provides access to public market data from Binance, allowing developers to retrieve information such as ticker prices, trading volume, and other essential details needed for trading applications.

Learn more about Market Data API at [github.com/l0v3m0n3y/binancevision](https://github.com/l0v3m0n3y/binancevision).

This is an unofficial SDK for the Market Data public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — MarketData — that you
...
| --- | --- | --- |
| TypeScript | `@voxgig-sdk/market-data` | publish pending — [install from git tag](https://github.com/voxgig-sdk/market-data-sdk/releases) |
...
// Load marketdata data (returns a MarketData)
const marketdata = await client.MarketData().load()
console.log(marketdata)
```
...
```bash
cd go-mcp && go build -o market-data-mcp .
```
...
```json
{
  "mcpServers": {
    "market-data": {
      "command": "/abs/path/to/market-data-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **MarketData** | The MarketData entity (load). | `/api/v3/ticker/24hr` |
...
# Load a specific marketdata (returns the record, raises on error)
marketdata = client.MarketData().load()
print(marketdata)
```
...
```go
import sdk "github.com/voxgig-sdk/market-data-sdk/go"

client := sdk.New()

// Load marketdata data
...
Generated from the Market Data API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).

...
[MCP](https://modelcontextprotocol.io) server exposing the MarketData SDK as
two agent tools — `market-data_list` and `market-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/market-data-mcp)
make build

# 2. Provide credentials via the environment
export MARKET_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user market-data \
  -- /absolute/path/to/market-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./market-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// market-data_list: first page of records
{ "entity": "market_data" }
{ "entity": "market_data", "query": { } }

// market-data_load: one record by id
{ "entity": "market_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/market-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user market-data \
     -- "$PWD"/dist/*/market-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `market-data_list` and `market-data_load` tools now appear
   in new sessions. Ask the agent to *"list market_data using market-data"*
   and it calls `market-data_list` with `{"entity":"market_data"}`.
...
```sh
export MARKET_DATA_APIKEY=sk_live_xxx            # API key
export MARKET_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `market-data_list` tool
...
### Call the `market-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `market-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `market-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MARKET_DATA_APIKEY` | API key sent with every request. |
| `MARKET_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./market-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"market-data_load","arguments":{"entity":"market_data","query":{"id":1}}}}'
```

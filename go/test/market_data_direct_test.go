...
func TestMarketDataDirect(t *testing.T) {
	t.Run("direct-load-market_data", func(t *testing.T) {
		setup := market_dataDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-market_data", _mode); _shouldSkip {
			if _reason == "" {
...
func market_dataDirectSetup(mockres any) *market_dataDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"MARKET_DATA_TEST_MARKET_DATA_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewMarketDataSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["MARKET_DATA_TEST_MARKET_DATA_ENTID"]; ok {

...
func TestMarketDataEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MARKETDATA_TEST_MARKET_DATA_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func market_dataBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MARKETDATA_TEST_MARKET_DATA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MARKETDATA_TEST_MARKET_DATA_ENTID"])

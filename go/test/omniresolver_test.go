...
import (
	...
	omni "github.com/voxgig-sdk/market-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MarketDataError); is {
				out := map[string]any{
...
func omniCtx(arg any, client *sdk.MarketDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MarketDataSDK); is {
			client = live

-- MarketData SDK MarketData entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MarketDataEntity = {}
MarketDataEntity.__index = MarketDataEntity


function MarketDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MarketDataEntity)
  self._name = "market_data"
  self._client = client
...
function MarketDataEntity:get_name()
  return self._name
end


function MarketDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MarketDataEntity.new(self._client, opts)
end
...
function MarketDataEntity:mark_deleted()
  self._deleted = true
end


function MarketDataEntity:deleted()
  return true == self._deleted
end


function MarketDataEntity:data_set(args)
  if args ~= nil then
...
function MarketDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MarketDataEntity:match_set(args)
  if args ~= nil then
...
function MarketDataEntity:match_get()
...
function MarketDataEntity:stream(action, args, callopts)
...
function MarketDataEntity:_run_op(ctx, post_done)
  local utility = self._utility

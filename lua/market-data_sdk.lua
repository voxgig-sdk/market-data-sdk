...
local MarketDataSDK = {}
MarketDataSDK.__index = MarketDataSDK
...
MarketDataSDK._make_feature = _make_feature


function MarketDataSDK.new(options)
  local self = setmetatable({}, MarketDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MarketDataSDK:options_map()
...
function MarketDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function MarketDataSDK:get_root_ctx()
  return self._rootctx
end


function MarketDataSDK:prepare(fetchargs)
...
function MarketDataSDK:direct(fetchargs)
...
-- by passing it.
function MarketDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MarketDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MarketDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MarketDataSDK:MarketData(data)
...
function MarketDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MarketDataSDK.new(sdkopts)
  sdk.mode = "test"
